...
atuin took the `mv` and stayed away. gh CAME BACK seven minutes later, with `version: "1"` in it,
and the culprit is in this repo: `git.nix` exports the token through `gh auth token` on EVERY
shell start, and gh writes a stub config when the file is missing. Every shell that opens between
the `mv` and the switch puts the conflict back, so a one-time move cannot win that race.

`xdg.configFile."gh/config.yml".force = true` is therefore declared, and the three conditions
that make it legitimate here are worth naming, because they are what separate this from papering
over a conflict:

1. the file is regenerated by a TOOL as a side effect, not authored by a human, so there is no
   intent to preserve
2. its entire content is declared, measured line by line, so nothing is lost by overwriting
3. a hook in this same repo recreates it, which makes the conflict recurring rather than a
   one-time adoption

The cost is real and accepted: `gh config set` now writes to a store symlink and fails. That is
rule 14 working as designed, Nix being the single owner, and not a bug to fix later.

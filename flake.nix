{
  ...
  inputs = {
    ...
    vscode-tarball = {
      url = "tarball+https://update.code.visualstudio.com/1.139.0/linux-x64/stable";
      flake = false;
    };
  };
  ...
  outputs =
    ...
    let
      ...
      # OpenRGB that knows the Arc B580: TEMPORARY. The channel ships 1.0rc3, which predates the
      # ASRock GPU controller entirely, so no version of it can drive this card. Master can, with
      # three patches on top: someone else's merge request for the Intel bus, plus the two that
      # came out of this machine. REMOVE when they land and the channel catches up, which also
      # deletes ./pkgs/openrgb: docs/notes/hardware/gpu-rgb.md
      overlayOpenrgbArc = _: prev: {
        openrgb = prev.openrgb.overrideAttrs (old: {
          version = "1.0-unstable-2026-09-18";
          src = prev.fetchFromCodeberg {
            owner = "OpenRGB";
            repo = "OpenRGB";
            rev = "cd44e41d462629410a806119694a773e88449ce1";
            hash = "sha256-0Hx23P6x9orNmCu+t14E2XyO04FS+KOJjf8dA9ZPoxQ=";
          };

          # NOT `old.patches ++`: the channel's own patch does not apply to master, so ours is a
          # rebase of it and REPLACES it. Dropping it instead would quietly break `withPlugins`.
          patches = [
            ./pkgs/openrgb/system-plugins-env.patch
            ./pkgs/openrgb/intel-gpu-i2c-bus.patch
            ./pkgs/openrgb/asrock-arc-b580-steel-legend.patch
            ./pkgs/openrgb/asrock-gpu-shutdown.patch
          ];

          # The channel patches a script that master DELETED: since 1.0 the udev rules are
          # generated by the binary itself, so the substitution has nothing to stand on.
          postPatch = "";

          # `lrelease` moved to qttools, and master's build calls it. The channel has qttools in
          # buildInputs, which does NOT put it on PATH, and qmake bakes qtbase's path into the
          # Makefile anyway, so the assignment has to happen after qmake has run.
          nativeBuildInputs = old.nativeBuildInputs ++ [ prev.qt6Packages.qttools ];
          postConfigure = ''
            sed -i "s|${prev.qt6Packages.qtbase}/bin/lrelease|${prev.qt6Packages.qttools}/bin/lrelease|g" Makefile
          '';

          # And this is the other half of that deletion: `make install` never installs rules, so
          # without this the package has no `lib/udev/rules.d` and `services.udev.packages`
          # silently installs nothing. Offscreen because the binary is a Qt app even here.
          # The `sed` is the channel's own substitution, moved: it used to patch the generator
          # script, and the strings now live inside the binary, so the only place left to catch
          # them is the output. NixOS refuses a rules file that references /usr/bin/env.
          postInstall = old.postInstall + ''
            install -d $out/lib/udev/rules.d
            QT_QPA_PLATFORM=offscreen $out/bin/openrgb --print-udev-rules \
              | sed "s|/usr/bin/env chmod|${prev.coreutils}/bin/chmod|g" \
